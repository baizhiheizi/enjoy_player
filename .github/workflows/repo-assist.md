...
network:
  ...
  - java
  - github
  # Flutter toolchain + pub ecosystem (ADR-0028 explicitly sanctions pub.dev egress)
  - dart
...
steps:
  ...
  # ==== customized start  ====  Flutter dev environment (mirrors ci.yml)
  - name: Ensure Linux tooling
    run: bash .github/scripts/ensure_linux_tooling.sh

  # Installs the Flutter version pinned in mise.toml (single source of truth
  # for local dev and CI). Installs persist in ~/.local/share/mise across
  # runs on this self-hosted runner, so the archive is fetched once per
  # version bump. mise-action is pinned by commit; `version` pins mise itself
  ...
  # before any read-only-mount window (see #501 and ci.yml).
  - name: Flutter version probe
    run: flutter --version
...
This is a Flutter repository. The workflow provisions a full dev environment before you start, mirroring CI:

- Flutter `3.44.0` (pinned in `mise.toml`) is installed via mise; `flutter` and `dart` are on your PATH. Do not download or bootstrap your own SDK.
- `flutter pub get` has already been run for the root package.
- Linux desktop build packages (GTK, mpv, SQLite, GStreamer dev headers, …) are installed (see `.github/scripts/ensure_linux_tooling.sh`).

Rules of the road:

- Re-run `flutter pub get` after switching branches; path packages under `packages/*` need their own `flutter pub get && flutter test` when you touch them.
- Every code change must end green: `bash .github/scripts/validate_ci_gates.sh --fix` (format + codegen + analyze + test) before creating or updating a PR — see AGENTS.md "Verification".
- Never hand-edit `*.g.dart` / `*.freezed.dart`; regenerate with `dart run build_runner build`.
- Network egress is firewalled (allowed: GitHub, pub.dev, npm, PyPI, crates.io and the standard language/package ecosystems). A connection timeout or refusal is the firewall, not a broken mirror — report it instead of working around it.
