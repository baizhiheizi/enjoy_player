...
void main() {
  ...
  group('2. document-generation invariants', () {
    ...
    test(
      'two noteWatchDocumentLoaded without noteVolumeRestored keeps '
    ...
    test('noteProgressForVolumeRestore re-bases the baseline on each advance',
        () {
  ...
  group('3. episode-keyed budget retirement', () {
    ...
    test(
      'budget armed through first playing, retires only after expiry '
      ...
      () {
        ...
        expect(
          ...
          isTrue,
          reason: 'arming survives the first playing — the latch is keyed '
  ...
  group('4. D9 direction latch', () {
    ...
    test(
      'no video element leaves the budget untouched — the attempt '
    ...
    test(
      'an unknown DOM direction leaves the latch untouched',
      () {
        final decision = YouTubePlayRetryPolicy()
            .classifyTransportToggle(domDirection: 'something_else');
        expect(decision, isA<LeaveRetryBudget>());
      },
    );

    test(
      'mirror race: a stale session.playing opposite to the DOM '
