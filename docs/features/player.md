...
- **Engine swap (ADR-0057)**: When switching YouTube ↔ MediaKit, `ensureEngineForPlayableSource` installs the new engine and bumps `playerEngineRevProvider` **before** disposing the old one so `PlayerSurfaceHost` can drop the prior platform view.
