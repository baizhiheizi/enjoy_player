...
Media persists in two sibling tables (`videos` / `audios`), but callers think in terms of one [`Media`](../lib/features/library/domain/media.dart). [`MediaRegistry`](../lib/data/db/media_registry.dart) is the single seam over that split: it owns the video-then-audio probe (exposed as `probeBoth` for callers needing full rows), the row→`Media` mapping (`mediaFromVideo` / `mediaFromAudio`), unified lookups (`getById`, `kindOf`, `dexieTargetTypeForId`, `localUriOf`), and — since issue #723 — the write dispatch: `upsertVideo` / `upsertAudio`, `deleteById`, `touchUpdatedAt`, `updateLanguage`, `updateLocalFile`, `patchDurationIfZero`, `updateVideoThumbnail`, plus the `syncEntityTypeOf` kind→entity mapping callers need to enqueue sync work. New code reads *and writes* media through the registry instead of re-probing or branching over both DAOs; transaction, GC, and enqueue policy stay with the callers (e.g. `MediaLibraryRepository`). Video-first precedence when an id exists in both tables is defensive and covered by tests. Known accepted bypasses: [`SyncDownloadService`](../lib/features/sync/data/sync_download_service.dart) / [`SyncUploadService`](../lib/features/sync/data/sync_upload_service.dart) internals legitimately stay on per-table DAO callbacks (documented on those classes).
