...
Media persists in two sibling tables (`videos` / `audios`), but callers think in terms of one [`Media`](../lib/features/library/domain/media.dart). [`MediaRegistry`](../lib/data/db/media_registry.dart) is the single seam over that split: it owns the video-then-audio probe (exposed as `probeBoth` for callers needing full rows), the row→`Media` mapping (`mediaFromVideo` / `mediaFromAudio`), unified lookups (`getById`, `kindOf`, `dexieTargetTypeForId`, `localUriOf`, `existsByLocalUri`), the kind-known single-table reads (`getVideoById`, `getAudioById`, `getAudioByMd5`, `getYoutubeVideoByVid`) for callers that already resolved the kind, the merged library stream `watchAll()` (dual-stream merge, `createdAt`-desc sort, identical-emission dedupe, and the empty-library first emission — issue #753 moved the glue here from `MediaLibraryRepository.watchAll`, whose relay issue #765 then deleted — callers now cross the registry directly via `mediaRegistryProvider`), and — since issue #723 — the write dispatch: `upsertVideo` / `upsertAudio`, `deleteById`, `touchUpdatedAt`, `updateLanguage`, `updateLocalFile`, `patchDurationIfZero`, `updateVideoThumbnail`, `updateYoutubeMetadata`, plus the `syncEntityTypeOf` kind→entity mapping callers need to enqueue sync work. New code reads *and writes* media through the registry instead of re-probing or branching over both DAOs; transaction, GC, and enqueue policy stay with the callers (e.g. `MediaLibraryRepository`). Video-first precedence when an id exists in both tables is defensive and covered by tests. Known accepted bypasses — exhaustive as of issue #753, every other `videoDao` / `audioDao` access outside `lib/data/db/` routes through the registry: (1) [`SyncDownloadService`](../lib/features/sync/data/sync_download_service.dart) / [`SyncUploadService`](../lib/features/sync/data/sync_upload_service.dart) internals legitimately stay on per-table DAO callbacks (documented on those classes); (2) the app-managed-media GC's *cross-file* probe in [`app_managed_media_gc.dart`](../lib/data/files/app_managed_media_gc.dart) runs raw `sqlite3` SQL over **other accounts'** per-user DB files, which a registry bound to one [`AppDatabase`](../lib/data/db/app_database.dart) cannot reach — mandated by [ADR-0050 §5](decisions/0050-path-linked-local-media.md), with the SQL single-sourced in `crossFileProbeLibraryTables` / `crossFileProbeLocalUriSql` and guarded by a schema-drift test against the Drift `TableInfo`s (the current-DB half of that check routes through `existsByLocalUri`). `SyncQueueJob.snapshotUpsert`'s `syncStatus → pending` flip is *not* an exception: its media arms cross the seam as kind-known reads plus the typed `upsertVideo` / `upsertAudio` replace.
