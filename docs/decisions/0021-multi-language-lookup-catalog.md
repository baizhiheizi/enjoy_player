...
4. **Source override** — add `resolveLookupSourceOverride(String?)` so the user can pick a source language inside the sheet even when the active transcript track is `und` / empty / denylisted. The existing `resolveLookupSource` from track language (or learning fallback) is preserved as the default when the user does not override.
