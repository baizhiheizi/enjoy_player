...
- **FR-010**: L1 MUST enforce both capacity (LRU eviction on overflow) and TTL (entry expires when `now - createdAt >= ttl`; a zero TTL expires immediately on read). Both MUST be testable in isolation.
